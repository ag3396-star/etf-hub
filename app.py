...
@st.cache_data(show_spinner=False, ttl=86400)   # refresh constituent list once per day
def _fetch_sp500_tickers() -> list[str]:
    """Return the current S&P 500 constituent tickers using yfinance — no external scraping.

    Strategy (all via yfinance, no Wikipedia / requests dependency):
      1. yf.Ticker('^GSPC').constituents  — available in yfinance ≥ 0.2.38
      2. yf.market.get_market_summary() screener fallback
      3. Minimal hard-coded seed list so the app never breaks entirely
    """
    # ── Attempt 1: index constituents via yfinance ────────────────────────────
    try:
        members = yf.Ticker("^GSPC").constituents          # returns a list/Index of tickers
        if members is not None and len(members) >= 400:
            tickers = [str(t).replace(".", "-") for t in members]
            return tickers
    except Exception:
        pass

    # ── Attempt 2: yfinance Screener (≥ 0.2.x) ───────────────────────────────
    try:
    ...
    # ── Attempt 3: iShares IVV holdings endpoint (plain HTTP — no scraping) ──
    try:
        ...
        df_h = pd.read_csv(_SI(resp.text), skiprows=9)
        col = next((c for c in df_h.columns if "ticker" in c.lower()), None)
        if col:
            tickers = (
                df_h[col]
                .dropna()
            ...
            tickers = [t for t in tickers if t and t not in ("-", "nan", "CASH")]
            if len(tickers) >= 400:
                return tickers
    except Exception:
        pass

    # ── Final fallback: large representative seed (avoids total failure) ──────
    return [
        "AAPL","MSFT","NVDA","GOOGL","AMZN","META","TSLA","BRK-B","JPM","V",
        "UNH","XOM","JNJ","WMT","MA","PG","HD","CVX","MRK","ABBV",
        "LLY","AVGO","PEP","KO","COST","TMO","MCD","ACN","BAC","CSCO",
        "ABT","CRM","ADBE","PFE","TXN","NFLX","DHR","CMCSA","LIN","AMD",
        "WFC","NEE","RTX","INTC","HON","AMGN","LOW","SPGI","INTU","QCOM",
    ]
...
def fetch_sp500_top6(months):
    ...
    # Build a set of current S&P 500 tickers for validation
    valid_ticker_set = set(sp500_tickers) | {BENCHMARK}
    ...
    for col in normalized_eq.columns:
        ...
        # ── Data quality gates ────────────────────────────────────────────────
        # 1. Must be a known current S&P 500 constituent or benchmark
        if col not in valid_ticker_set:
            continue
        ...
        # 4. Drop any single monthly return > 150% (almost always a data artifact
        #    from a delisting event, reverse merger, or bad yfinance adjustment)
        if mr.abs().max() > 1.50:
            continue
        ...
        # 5. Cap annualized return sanity check: >400% p.a. for >6m window is
        #    almost certainly a bad data point (SNDK-type artifacts)
        if months >= 6 and ann > 400:
            continue
...
def render_sp500_section(months, selected_tf):
    ...
    st.markdown(
        f"<p style='color:#94a3b8;font-size:13px;'>Top 6 S&P 500 companies by <b style='color:#38bdf8;'>Annualized Return</b> over <b style='color:#38bdf8;'>{selected_tf}</b>, compared to VOO benchmark. Scatter shows top 50 by annualized return.</p>",
        unsafe_allow_html=True
    )
