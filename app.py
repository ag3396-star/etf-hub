...
# ── JAVASCRIPT: per-table sort + live search ────────────────────────────────
# Each table gets its own self-contained <script> injected in the same HTML
# block so it runs inside the correct Streamlit iframe.
# No shared globals, no cross-frame issues, no _si guard needed.

def _table_js(table_id):
    """Return a self-contained <script> that wires sort+filter for one table."""
    return f"""
<script>
(function(){{
  var TID = '{table_id}';
  function boot(){{
    ...
    if(!tbl) return false;
    if(tbl.dataset.ready) return true;
    tbl.dataset.ready = '1';
    ...
    ths.forEach(function(th, ci){{
      ...
      th.addEventListener('click', function(){{
        ...
        ths.forEach(function(h, i){{
          ...
          if(icon) icon.textContent = (i===ci) ? (sortAsc?'▲':'▼') : '⇅';
        }});
        var rows = Array.prototype.slice.call(tbody.rows);
        rows.sort(function(a,b){{
          var at = a.cells[ci] ? a.cells[ci].textContent.trim() : '';
          var bt = b.cells[ci] ? b.cells[ci].textContent.trim() : '';
          var an = parseFloat(at.replace(/[^0-9.e+-]/g,''));
          var bn = parseFloat(bt.replace(/[^0-9.e+-]/g,''));
    ...
    function filter(){{
      ...
      Array.prototype.slice.call(tbody.rows).forEach(function(row){{
        ...
        var ok = filters.every(function(f,ci){{
          if(!f) return true;
  ...
  /* retry until the table element exists (Streamlit lazy-renders) */
  var tries=0;
  var iv = setInterval(function(){{
    if(boot() || ++tries>80) clearInterval(iv);
  }}, 100);
}})();
</script>"""


# Keep a no-op placeholder so old call sites don't break
SORTABLE_TABLE_JS = ""  # JS is now injected per-table via _table_js()
...
def _make_table(table_id, col_labels, rows_html):
    """Returns full HTML for a sortable+searchable table with embedded JS."""
    hdr = ""
    ...
    # Embed the per-table JS directly — runs inside the same Streamlit iframe
    return (
