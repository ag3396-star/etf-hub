...
def fetch_sp500_top6(months):
    ...
    for col in normalized_eq.columns:
        ...
        #    from a delisting/acquisition event or bad yfinance adjustment.
        #    e.g. SNDK: delisted 2016 after WD acquisition — yfinance returns a
        #    huge spike on the last trading day which is NOT a real investable return)
        if mr.abs().max() > 1.50:
            continue
...
def render_sp500_section(months, selected_tf):
    ...
    st.subheader("📊 Top 25 S&P 500 Stocks — Annualized Return")
    ...
    fig_bar.add_trace(go.Bar(
        x=bar_df["Ticker"],
        y=bar_df["Annualized Return %"],
        marker=dict(color=bar_colors, line=dict(width=0)),
        text=[f"{v:.1f}%" for v in bar_df["Annualized Return %"]],
    ...
    if bench_data is not None:
        fig_bar.add_hline(
            y=bench_data["Annualized Return %"],
            line=dict(color="#ef4444", width=2, dash="dash"),
            annotation_text=f"VOO {bench_data['Annualized Return %']:.1f}%",
