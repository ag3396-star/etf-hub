...
@st.cache_data(show_spinner=False, ttl=3600)   # refresh every hour
def fetch_sp500_top6(months):
    """Fetch live S&P 500 tickers via Wikipedia (same method as yfinance_top50.py),
    download price data, compute metrics for every constituent, return top-6 + VOO.

    No secondary membership filter — every ticker successfully downloaded is kept.
    This mirrors the logic in yfinance_top50.py exactly, so the rankings match.
    """
    import requests as _req
    from io import StringIO as _SI

    # ── Step 1: Get the full S&P 500 ticker list ──────────────────────────────
    # Primary: Wikipedia — the same source that yfinance_top50.py uses and that
    # reliably returns ~503 tickers.  The historyofmarket.com changes overlay is
    # applied on top to catch recent additions (SNDK, KLAC re-add, etc.).
    sp500_tickers = []
    ...
    # Apply live changes overlay from historyofmarket.com so recent additions
    # (e.g. SNDK added Nov 2025, LITE/COHR/VRT added Mar 2026) are included
    # and recent removals are excluded — making the list current as of today.
    if sp500_tickers:
        try:
            ...
            baseline = set(sp500_tickers)
            for entry in changes_resp.json().get("changes", []):
                try:
                    eff_date = _date.fromisoformat(entry.get("effectiveDate", ""))
                except ValueError:
                    continue
                if eff_date > today:
                    continue
            ...
            sp500_tickers = sorted(baseline)
        except Exception:
            pass  # keep the Wikipedia list as-is if the overlay fails

    # Fallback: yfinance native index object
    if len(sp500_tickers) < 400:
        try:
            members = yf.Ticker("^GSPC").constituents
            if members is not None and len(members) >= 400:
                sp500_tickers = [str(t).replace(".", "-") for t in members]
        except Exception:
            pass

    # Last-resort seed — only reached if both network calls fail entirely
    if len(sp500_tickers) < 400:
        sp500_tickers = [
            ...
            "HON","AMGN","LOW","SPGI","INTU","QCOM","SNDK","KLAC","LITE","MU",
            "SATS","STX","CIEN","COHR","VRT","APP","CVNA","CRH","TER","IBKR",
        ]

    # ── Step 2: Download price data — exactly like yfinance_top50.py ─────────
    end_date   = datetime.today()
    start_date = end_date - relativedelta(months=months + 1)
    ...
    df_close = raw["Close"] if isinstance(raw.columns, pd.MultiIndex)                else pd.DataFrame({dl_tickers[0]: raw["Close"]})
    ...
    # ── Step 3: Compute metrics for every downloaded ticker ───────────────────
    # No secondary membership gate — if yfinance returned data for it, it counts.
    min_obs   = max(2, int(months * 0.6))
    norm      = monthly / monthly.iloc[0]
    rf_mo     = RF_ANNUAL / 12
    n_years   = months / 12.0
    results   = []
    ...
    for col in norm.columns:
        ...
        # Block only physically impossible single-month moves (> 500%).
        # Tight caps (e.g. 150%) wrongly exclude genuine high-flyers like MU, SNDK.
        if mr.abs().max() > 5.0:
            continue

        total = norm[col].dropna().iloc[-1]
        ann   = (total ** (1 / n_years) - 1) * 100 if total > 0 else 0

        # Cap only for very long windows where > 5000% p.a. is truly impossible.
        ...
        dn = mr[mr < 0]
        if len(dn) < 2:
            sortino = min((exc.mean() * 12) / (vol) * 1.5, 20.0) if vol > 0.0001 else 0
        else:
            dv = dn.std() * np.sqrt(12)
    ...
    # Rank top-6 by Annualized Return % (or Total Return for short windows —
    # the display layer picks the right column via ret_col)
    top6 = non_bench.sort_values("Annualized Return %", ascending=False).head(6)

    # Top-50 for scatter plot
    top50_tickers = set(
        non_bench.sort_values("Annualized Return %", ascending=False).head(50)["Ticker"]
    )
    bench_df = metrics_eq[metrics_eq["Ticker"] == BENCHMARK]
    metrics_eq_scatter = pd.concat([
        metrics_eq[metrics_eq["Ticker"].isin(top50_tickers)],
        bench_df,
    ]).drop_duplicates("Ticker").reset_index(drop=True)

    # Fetch market cap + sector for top-6 + benchmark
    cap_tickers  = list(top6["Ticker"]) + [BENCHMARK]
    mcap_map     = _fetch_mcap(tuple(cap_tickers))
    sector_map   = _fetch_sector(tuple(top6["Ticker"]))
    top6         = top6.copy()
    top6["Market Cap"] = top6["Ticker"].map(mcap_map).fillna("N/A")
    top6["Sector"]     = top6["Ticker"].map(sector_map).fillna("S&P 500")
    if not bench_df.empty:
        metrics_eq_scatter.loc[
            metrics_eq_scatter["Ticker"] == BENCHMARK, "Market Cap"
        ] = mcap_map.get(BENCHMARK, "N/A")

    return top6, metrics_eq_scatter, norm, sp500_tickers
@st.cache_data(show_spinner=False)
...
@st.cache_data(show_spinner=False)
def _fetch_sector(tickers: tuple) -> dict:
    """Return {ticker: sector_string} using yfinance .info dict."""
    result = {}
    for tk in tickers:
        try:
            info = yf.Ticker(tk).info
            sector = info.get("sector", None) or info.get("industry", None)
            result[tk] = sector if sector else "S&P 500"
        except Exception:
            result[tk] = "S&P 500"
    return result
...
def render_sp500_section(months, selected_tf):
    ...
    with st.spinner("Fetching S&P 500 stock data…"):
        top6, metrics_eq, normalized_eq, sp500_list = fetch_sp500_top6(months)
    n_universe  = len(sp500_list)
    n_qualified = len(metrics_eq[metrics_eq["Ticker"] != BENCHMARK]) if not metrics_eq.empty else 0
    st.caption(f"📋 Universe: {n_universe} S&P 500 tickers fetched · {n_qualified} passed data quality checks")
    ...
    for idx, (_, row) in enumerate(top6.iterrows()):
        ...
        with cols_eq[idx]:
            ...
            <div class="circle-card" style="border:3px solid {bc};">
                <div class="circle-ticker">{tk}</div>
                <div class="circle-meta">{row.get('Sector', 'S&P 500')}</div>
