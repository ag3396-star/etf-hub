...
def fetch_sp500_top6(months):
    ...
    try:
        ...
        # Fix common ticker format issues (BRK.B → BRK-B etc.)
        wiki_tickers = [t.replace('.', '-') for t in wiki_tickers]
    except Exception:
        wiki_tickers = ["AAPL","MSFT","NVDA","GOOGL","AMZN","META","TSLA","BRK-B","JPM","V"]

    end_date   = datetime.today()
    start_date = end_date - relativedelta(months=months + 1)
    dl_tickers = list(set(wiki_tickers + [BENCHMARK]))
    ...
    valid_ticker_set = set(wiki_tickers) | {BENCHMARK}
    ...
    # Rank top-6 by Sharpe (risk-adjusted quality), not raw return,
    # so short-timeframe outliers don't dominate the circles
    non_bench = metrics_eq[metrics_eq["Ticker"] != BENCHMARK].copy()
    top6 = non_bench.sort_values("Sharpe Ratio", ascending=False).head(6)
...
def render_sp500_section(months, selected_tf):
    ...
    st.markdown(
        f"<p style='color:#94a3b8;font-size:13px;'>Top 6 S&P 500 companies by <b style='color:#38bdf8;'>Sharpe Ratio</b> over <b style='color:#38bdf8;'>{selected_tf}</b>, compared to VOO benchmark. Ranked by risk-adjusted return to exclude short-term data artifacts.</p>",
        unsafe_allow_html=True
    )
    ...
    st.subheader("🎯 S&P 500 Risk-Adjusted Landscape (Sharpe vs Annualized Return)")
